...
class BusinessUnit:  # подразделение фирмы
    ...
    def take_to_the_unit(self, office_eq, num=1, get_from=None):
        ...
        if get_from:
            if get_from.__class__ is not BusinessUnit and not issubclass(get_from.__class__, BusinessUnit):
                raise ValueError('get_from должно быть None или экземпляром BusinessUnit или Warehouse!')
            if not hasattr(get_from, '__class__') or not issubclass(office_eq.__class__, OfficeEquipment):
                raise ValueError('Передайте в качестве параметра единицу оргтехники!')
            if office_eq not in get_from.__equipment.keys():
                raise ValueError(f'Такой техники ({office_eq.get_description()}) нет в: ' + get_from.__name)
...
shr = Shredder(maker='BRAUBERG', model='S8', production_date=2015, item_number='000031',
               serial_number='663254100993', speed='8 стр/мин', year_of_receipt=2015)
warehouse = Warehouse('Склад')
warehouse.take_to_the_unit(pr, 2)
warehouse.take_to_the_unit(shr, 1)
...
try:
    warehouse.take_to_the_unit(pr, 2, 'склад')  # попытка переместить из подразделения 2 на склад принтер, которого там нет
except ValueError as err:
    print(err)
try:
    warehouse.take_to_the_unit('принтер', 2, bu2)  # попытка переместить из подразделения 2 на склад принтер, которого там нет
except ValueError as err:
    print(err)
print('---------------------------------------------------------')
bu2.take_to_the_unit(pr, 1, warehouse)  # перемещаем со склада в подразделение 2 принтер
try:
    bu2.take_to_the_unit(pr, 2, warehouse)  # попытка переместить со склада в подразделение 2  два принтера,
    # а на складе остался только один
except ValueError as err:
    print(err)
print(warehouse)
print(bu1)
